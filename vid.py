def menu():
    ...
    '''
----------------------------------------------------------
Üdvözöljük a Szám konverterben!
A rendszer a következőket fogja kérni:
- A konvertálandó szám
- Milyen alapról szeretnél konvertálni
- Milyen alapra szeretne konvertálni
** NE felejtse el: Minden számnak egész számnak kell lennie!  **
----------------------------------------------------------
    ''')
...
def execute_converter():
    ...
    while proceed.lower() == 'i':
        valid_input = False
        while valid_input == False:
            user_number = input('\nÍrd be milyen számot akarsz átváltani: ')
            user_input_base = input('Írd be az alapszámrendszert amibe a számod van(Pl.10): ')
            user_output_base = input('Írd be melyik számrendszerbe akarod átváltani(Pl.2): ')

            valid_input = validator(user_number,user_input_base,user_output_base)

        print (f'\nA(z) {user_number} számot Base-{user_input_base}-ről Base-{user_output_base}-re próbálom konvertálni: ')
        print (f'>> Az eredmény: {convert_number_system(user_number, user_input_base, user_output_base)} <<')

        print (f'\nÁt akarsz váltani még egy számot? Ird be i vagy n: ')
        proceed = input('')
